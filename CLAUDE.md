...
- **`Vision.ts`** — `VisionService.scanWorksheet(image, mimeType)` posts a JPEG/PNG to OpenAI `gpt-4o-mini` (vision) with `response_format: json_object` and returns `{name, emoji, description, spec}`. The `spec` is fed straight back through `/api/apps` create — the agent treats it as any other prompt. Powers the home **Scan a worksheet** button: kid photographs a printed maths sheet / coloring page / maze and gets an interactive web-app version. Camera capture lives in `KidScanOverlay`; Vision only does the LLM call.
