...
- **`Vision.ts`** — `VisionService.scanWorksheet(image, mimeType)` stages the photo in a unique temp dir, then spawns `claude --print --model opus --tools Read --json-schema …` with that dir as cwd. Claude reads the staged file via the Read tool and returns `{name, emoji, description, spec}`; the `spec` feeds straight back through `/api/apps` create. CLI instead of the Anthropic API so a fresh checkout works without `ANTHROPIC_API_KEY` (claude code login is enough). Override the model with `MOCHI_VISION_MODEL=sonnet` if Opus's latency / cost feels heavy. Powers the home **Scan a worksheet** button; camera capture lives in `KidScanOverlay`.
